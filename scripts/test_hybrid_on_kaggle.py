"""Test the hybrid KTH-only model on Kaggle Real Fall Dataset."""
...
class HybridFallDetector(nn.Module):
    """Hybrid model combining autoencoder and classifier."""
    
    def __init__(self, pretrained_autoencoder=None):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, return_features: bool = False):
        ...
        if return_features:
            ...
            return reconstructed, logits, latent, recon_error
        
        return reconstructed, logits


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test hybrid KTH model on Kaggle dataset")
    parser.add_argument(
        "--checkpoint",
        type=Path,
...
def test_model(args: argparse.Namespace):
    ...
    print("=" * 70)
    print("TESTING HYBRID KTH-ONLY MODEL ON KAGGLE DATASET")
    print("=" * 70)
    ...
    print(f"   Training accuracy: {checkpoint.get('val_accuracy', 'N/A'):.4f}")
    ...
    with torch.no_grad():
        for file_path, label in tqdm(zip(test_files, test_labels), total=len(test_files)):
            try:
                ...
                # Inference
                reconstructed, logits, latent, recon_error = model(tensor, return_features=True)
                ...
                all_recon_errors.append(recon_error.item())
    ...
    fall_errors = recon_errors[y_true == 1]
    no_fall_errors = recon_errors[y_true == 0]
    ...
    results_file = args.output_dir / "hybrid_kth_test_results.json"
    ...
    try:
        ...
        plt.title('ROC Curve (Hybrid KTH Model)')
        plt.legend()
        ...
        plt.subplot(2, 3, 5)
        plt.hist(no_fall_errors, bins=30, alpha=0.5, label='No Fall', color='blue', density=True)
        plt.hist(fall_errors, bins=30, alpha=0.5, label='Fall', color='red', density=True)
        ...
        # Scatter: Reconstruction error vs Probability
        plt.subplot(2, 3, 6)
        plt.scatter(recon_errors[y_true == 0], y_probs[y_true == 0], 
                   alpha=0.3, s=10, c='blue', label='No Fall')
        plt.scatter(recon_errors[y_true == 1], y_probs[y_true == 1], 
                   alpha=0.3, s=10, c='red', label='Fall')
        plt.xlabel('Reconstruction Error')
        ...
        plt.tight_layout()
        plot_file = args.output_dir / "hybrid_kth_metrics.png"
    ...
    print("SUMMARY")
    print("=" * 70)
    print(f"\n✅ Hybrid KTH-Only Model Performance:")
    ...
    print(f"   • Method: Synthetic falls + Hybrid architecture")
    
    # Comparison
    print(f"\n📊 Comparison with Other Models:")
    print(f"   Model                    Accuracy    F1 Score   False Alarms")
    print(f"   {'─' * 60}")
    print(f"   KTH (Anomaly)            45.58%      0.6211     98.23%")
    print(f"   Fine-tuned (Kaggle)      95.82%      0.9533      3.42%")
    print(f"   Hybrid (KTH-Only)        {accuracy*100:5.2f}%      {f1:.4f}     {(fp/(fp+tn))*100:5.2f}%  ⭐")
    
    if f1 >= 0.8:
        print(f"\n🎉 Excellent performance! (F1 ≥ 0.80)")
        print(f"   The synthetic fall approach is highly effective!")
    elif f1 >= 0.7:
        print(f"\n👍 Good performance! (F1 ≥ 0.70)")
        print(f"   Synthetic falls provide reasonable generalization.")
    elif f1 >= 0.6:
        print(f"\n👌 Acceptable performance (F1 ≥ 0.60)")
        print(f"   Better than anomaly detection, but room for improvement.")
    else:
        print(f"\n⚠️  Performance needs improvement (F1 < 0.60)")
        print(f"   Consider:")
        print(f"   • More sophisticated synthetic fall generation")
        print(f"   • Training on all KTH activities (not just walking)")
        print(f"   • Adding temporal LSTM layer")
